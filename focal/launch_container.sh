...
if [ ! $# -ne 1 ]; then
	if [ "commit" = $1 ]; then
		...
		docker commit nvidia_egl_focal_desktop_docker nvidia_egl_focal_desktop_ws:latest
		CONTAINER_ID=$(docker ps -a | grep nvidia_egl_focal_desktop_docker | awk '{print $1}')
...
DOCKER_OPT=""
DOCKER_NAME="nvidia_egl_focal_desktop_docker"
...
DOCKER_OPT="${DOCKER_OPT} \
		...
		-e NOVNC_ENABLE=true -e SSL_ENABLE=false -e CERT_PATH="~/" \
		-p $(id -u):8080 \
